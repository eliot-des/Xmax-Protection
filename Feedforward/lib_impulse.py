...
.. note:: Note that this module is covered by the GPLv3 license.
"""
...
def h1_z_deriv(n, p, ts):
    ...
    # Find {-zd/dz}^n*H1(z). I.e., first differentiate, then multiply by -z, then
    # differentiate, etc. The result is
    # (ts^(n+1))*(b(1)*p/(z-p)^1 + b(2)*p^2/(z-p)^2 + b(n+1)*p^(n+1)/(z-p)^(n+1)).
    # Works for n>0.
    """
    # Build the vector d that holds coefficients for all the derivatives of H1(z)
    # The results reads d(n)*z^(1)*(d/dz)^(1)*H1(z) + d(n-1)*z^(2)*(d/dz)^(2)*H1(z) +
    # ...+ d(1)*z^(n)*(d/dz)^(n)*H1(z)
...
def h1_deriv(n):
    ...
    b = (
        factorial(n) * bincoeffs
    )  # Binomial coefficients: [1], [1 1], [1 2 1], [1 3 3 1], etc.
    b *= (-1) ** n
    return b
...
def z_res(r_in, sm, ts):
    ...
    for i in range(1, n):  # Go through s-domain residues for multiple pole
        r_out[:i] += r_in[i] * polyrev(
            h1_z_deriv(i, p_out, ts)
        )  # Add z-domain residues
...
def impinvar(b_in, a_in, fs=1, tol=0.0001):
    ...
    # Copyright (c) 2016 Sascha Eichstädt <sascha.eichstaedt@ptb.de>
    #
    # This program is free software; you can redistribute it and/or modify it under
    # the terms of the GNU General Public License as published by the Free Software
    # Foundation; either version 3 of the License, or (at your option) any later
    ...
    while i < n:
        ...
        while (
            i < (n - 1) and np.abs(first_pole - p_in[i + 1]) < tol
